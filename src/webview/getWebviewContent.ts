...
<body style="margin:0;padding:0;background:#1e1e1e;font-family:system-ui,-apple-system,sans-serif;">
  ...
  <div id="app">
    ...
    <div id="project-context-menu" class="context-menu" style="display: none;">
      ...
      <div class="context-menu-separator"></div>
      <div class="context-menu-item" data-action="runListed" tabindex="0">▶ Run Listed Specs</div>
      <div class="context-menu-item" data-action="runAll" tabindex="0">▶ Run All Project Specs</div>
    </div>
  ...
  return `
    (function() {
        ...
        item.addEventListener('click', () => {
          ...
          switch (action) {
            case 'runListed':
              ...
              break;
            case 'runAll':
              send('runAllProjectSpecs', { projectName: contextMenuProject.name });
              break;
          }
          hideProjectContextMenu();
        });
      ...
      elements.outputStructured.addEventListener('click', (e) => {
        ...
        console.log('[ET WebView] Structured output click, target:', target.tagName, target.className);
        ...
        // Check if clicked on a header (the PASS/FAIL line with file path)
        ...
        // Also check if clicked directly on .structured-result container
        const resultEl = target.closest('.structured-result');
        if (resultEl && !target.closest('.structured-tests')) {
          console.log('[ET WebView] Result container clicked');
          const relPath = resultEl.dataset.file;
          const filePath = resolveStructuredFilePath(relPath);
          if (filePath) {
            send('openFile', { filePath });
            console.log('[ET WebView] Opening file:', filePath);
